resource "massdriver_artifact" "azure_storage_account_data_lake" {
  ...
  name                 = "Azure Data Lake Storage Account ${var.md_metadata.name_prefix} (${azurerm_storage_account.main.id})"
  artifact = jsonencode(
    {
      data = {
        infrastructure = {
          ari      = azurerm_storage_account.main.id
          endpoint = azurerm_storage_account.main.primary_dfs_endpoint
        }
        security = {
          iam = {
            "read" = {
